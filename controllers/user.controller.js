...
const SignupImages = db.signupimages;
...
exports.postSignupImages = async (req, res) => {
  const data = req.body;
  const schema = Joi.object({
  ...
  if (validationResult.error) {
    ...
    try {
      if (req.file) {
        const images = new SignupImages({ position: data.position, type: data.type });
        const imagePath = req.file.path;
        var imageUrl = imagePath.replace(/\\/g, "/");
        imageUrl = imageUrl.replace("public/", "");
        ...
        await images.save();
        res.status(201).json({
          message: "Signup Image created successfully",
          data: images,
        });
      }
    } catch (error) {
      console.error("Failed to add Signup Image:", error);
      res.status(500).json({ message: "Internal server error" });
    }
  }
...
exports.putSignupImages = async (req, res) => {
  const data = req.body;
  const schema = Joi.object({
    position: Joi.number().required(),
    type: Joi.string().required(),
  ...
  if (validationResult.error) {
    ...
    try {
      ...
      const img = await SignupImages.findById(id);
      if (!img) {
      ...
      console.error("Failed to update Signup Image:", error);
      res.status(500).json({ message: "Internal server error" });
    }
  }
...
exports.getSignupImages = async (req, res) => {
  try {
    const signupimages = await SignupImages.find().exec();
    ...
    console.error("Failed to fetch Signup Images:", error);
    res.status(500).json({ message: "Internal server error" });
  }
};
