...
exports.forgotPassword = (req, res) => {
  ...
  if (validationResult.error) {
    ...
    User.findOne({
      ...
      const password = resetToken;
      const salt = bcrypt.genSaltSync(8);
      user.password = bcrypt.hashSync(password, salt);
      user.resetToken = resetToken;
      user.save();

      sendPasswordNewEmail(user, resetToken);
      res.status(200).send({
        message:
          "An email with instructions for a new password has been sent to you.",
      });
    });
  }
};
...
function sendPasswordResetEmail(user, resetToken) {
  ...
  const transporter = nodemailer.createTransport({
    ...
    port: 587, // Port for SMTP (587 for TLS/STARTTLS, 465 for SSL)
    secure: false, // true for 465, false for other ports
    auth: {
        user: 'ashishSmartData@gmail.com', // Your email address
        pass: 'unal srgd ghgx lhsl' // Your email password
    }
  });

  const mailOptions = {
    from: "ashishSmartData@gmail.com",
    to: user.email,
...
function sendPasswordNewEmail(user, newPass) {
  // Replace the following email setup with your actual email configuration (e.g., SMTP, service, etc.)
  const transporter = nodemailer.createTransport({
    host: 'email-smtp.ap-southeast-2.amazonaws.com', // SMTP server hostname
    port: 587, // Port for SMTP (587 for TLS/STARTTLS, 465 for SSL)
    secure: false, // true for 465, false for other ports
    auth: {
        user: 'AKIAVRUVUV66RTMBI6XU', // Your email address
        pass: 'BIERBHKRom8CMBDQH/+GBhxr/kvsjhbsBLIbLSVQR2hW' // Your email password
    }
  });

  const mailOptions = {
    from: "notification@speedquizz.com",
    to: user.email,
    subject: "New password on Speed Quizz",
    html: `<p>Dear ${user.name},</p>

    <p>We are pleased to inform you that a new password has been generated for your account.</p></br>
    
    <p>Your new password is: <b>${newPass}</b></p></br>
    
    <p>For security reasons, we recommend changing this password after your first login.</br> You can change your password from the application once you have logged in.<p></br>
    
Thank you,<br />
The Speed Quizz Team
`,
  };
